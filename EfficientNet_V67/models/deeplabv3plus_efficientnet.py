...
def _apply_dilation_to_efficientnet(features: nn.Sequential, start: int, dilation: int) -> None:
    """
    EfficientNet features[start:] 블록에 dilation을 적용하여 output stride를 유지합니다.

    B0/B1 모두 stage 분할(features[]) 구조 동일 (B1은 width=1.0, depth=1.1 → 채널 동일):
      features[:3]  → stride 4  (24ch, low-level feature)
      features[3:6] → stride 16
      features[6:]  → stride 32 ← 여기서 stride=2가 한 번 더 발생

    features[6]의 stride=2를 stride=1 + dilation=2로 교체 → output stride 16 유지.
...
class DeepLabV3Plus_EfficientNet(nn.Module):
    """
    EfficientNet-B1 백본 기반 DeepLabV3+ (V7).

    B1은 B0 대비 width=1.0(동일), depth=1.1 → 채널 수는 동일, 레이어 수만 +10%.
    따라서 ASPP / Decoder 채널 인자 변경 불필요.
    ...
    def __init__(self, num_classes: int = 21) -> None:
        ...
        enet = models.efficientnet_b1(weights='IMAGENET1K_V1').features
        ...
        # EfficientNet-B1 마지막 채널: 1280ch (B0와 동일)
